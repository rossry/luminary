...
| `luminary/vibe/coder.py`, `session.py`, `setup.py` | — | The coding model. `Coder`: one Messages API call (httpx; injectable transport) with the system prompt that carries the pattern contract, the library inventory and the craft rules, and the rule that every prompt ships a best try (optional ≤2-sentence note, then one fenced block; `extract`). `SessionCoder`: one Claude Code session per prompt through the Agent SDK — read-only tools in the checkout, a `ship_pattern` MCP tool that runs the core's validator in-turn and returns what broke, so the session iterates before it answers; auto permission mode (the allow/deny lists resolve first, the classifier only sees a call outside both, nothing waits on a prompt; `LUMINARY_VIBE_PERMISSION_MODE` overrides — `bypassPermissions` is refused by the CLI under root); the runner is injectable (tests never spawn the CLI). `setup.default_coder` picks session when the SDK and a `claude` CLI are present (`LUMINARY_VIBE_BACKEND` forces), `vibe_models` the select list (`LUMINARY_VIBE_MODELS`) |

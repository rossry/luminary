...
| `luminary/drivers/broadcast.py` | §12.2, §12.3, §16.2.8 | `luminary show`: **one** engine to serial *and* preview viewers. The serial loop runs on its own thread and keeps pacing authority (§11.7.6); `FrameHub` fans the same frames to viewers over bounded queues that drop rather than delay. Contrast `/api/play`, which builds an engine per viewer — correct for browsing, wrong for an installation |
| `luminary/geometry/pentagon/mapped.py` | §7.3.1, §16.2.8 | **Closes §19.6**: mapping records → deployed geometry with real `(controller, channel, index)`. Canonical order makes each board one contiguous block, ascending by channel, each channel's indices 0..n-1 — so the frame path needs no dict lookup or reordering, and the SESSION's declared per-channel lengths let the board slice one flat buffer by offset |
| `luminary/mapping/strip_path.py` | `plan/mapping/DESCRIPTION.md` "The strip path" | The serpentine path and the strip-index→beam bridge, shared by the mapping session and the production capture — one implementation, so a deployment cannot render differently from the tool that mapped it |
...
| `luminary/stage/web.py` | — | Stage web adapter (`/stage`, `/api/queue[/play_next…]`, `/api/repeats/…`, `/api/audio`, `WS /api/stage` + layout/patterns/chapters): thin routes over `StageCore`, mapping-web-shaped per-socket sinks and fps ticker that idles when no sinks + no audio + queue exhausted; mutating routes gated by `X-Stage-Key` when a stage key is configured (`--stage-key` / env `LUMINARY_STAGE_KEY`, flag wins; read-only never gated); `--stage-lights` resolver (default: the pentagon-4A-33 capture) |
| `luminary/server/static/stage.html`, `stage.js` | — | The stage page: canvas viewer (imports `StreamView`/`WireStream` from `mapping.js`) + queue/repeats panels, add + play-next controls, chapter-path + liner-notes header, server-fetched chapter previews for queued compositions, stage-key field (localStorage / `#key=…` fragment); renders `/api/queue` state and sends commands, ~2 s poll — no client-side scheduling |
| `luminary/vibe/core.py` | — | `VibeCore`: vibe mode's shared state — prompts numbered at submit (#1 onward, never reused), the pattern showing *at submit* captured as context (name + source), one worker cooking in order: coder draft → identity stamp (`assign_identity`, AST: `name = "vibe-N"` + `description`) → `validate` (throwaway registry on the stage's own lights: traceback, `(n,3)` shape, finite, stateless, frame ≤ 0.25 s) → one repair round → `var/vibe/vibe-NNNN.py` (never overwritten; rejected drafts kept as `_vibe-NNNN-<tag>.py`, which the registry skips) → registry reload → `StageCore.cut`. `log.json` is the thread (tmp+rename); queued/cooking entries are marked failed on restart, never dropped. The menu groups the repo's patterns by folder from the registry's origins |
| `luminary/vibe/coder.py`, `session.py`, `setup.py` | — | The coding model. `Coder`: one Messages API call (httpx; injectable transport) with the system prompt that carries the pattern contract, the library inventory and the craft rules, and the rule that every prompt ships a best try (optional ≤2-sentence note, then one fenced block; `extract`). `SessionCoder`: ONE ongoing Claude Code session through the Agent SDK's `ClaudeSDKClient`, prompts fed in turn on its own event-loop thread (the worker blocks on each) — read-only tools in the checkout, a `ship_pattern` MCP tool that runs the core's validator in-turn and returns what broke, so it iterates before it answers, and memory across prompts (a generation it shipped itself is referred to by number, not re-sent; every generation is on disk for it to Read); a prompt past its wall-clock cutoff or a broken session is dropped and lazily restarted, an idle session (20 min) is closed, the model switches in place per submitter; auto permission mode (the allow/deny lists resolve first, the classifier only sees a call outside both, nothing waits on a prompt; `LUMINARY_VIBE_PERMISSION_MODE` overrides — `bypassPermissions` is refused by the CLI under root); the session factory is injectable (tests never spawn the CLI). `setup.default_coder` picks session when the SDK and a `claude` CLI are present (`LUMINARY_VIBE_BACKEND` forces), `vibe_models` the select list (`LUMINARY_VIBE_MODELS`) |
| `luminary/vibe/web.py`, `server/static/vibe.html`, `vibe.js` | — | Vibe web adapter: `GET /vibe`, `GET /api/vibe` (the whole snapshot), `POST /api/vibe` (202 + the numbered entry), `POST /api/vibe/select`; mutations share the stage's `stage_key_guard`. The page decodes the stage's own wire stream (no second render) and re-renders the snapshot every ~2 s and after each verb; menu (named / all / repo by folder, hideable), thread (status, note, context "while #N was showing"), prompt + name + author + model select + from-scratch. Mounted by `create_app` only when the stage is on and (`allow_pattern_upload` or a stage key) — the upload security model, §15.5.2 |
...
- `var/` (contents gitignored): the server's content-addressed geometry
  ...
  `patterns-uploads/` — plus stage state and audio, and vibe mode's
  generations (`vibe/vibe-NNNN.py` + `vibe/log.json`, a registry volume). Ships empty; ids are short SHA-1 content hashes, so

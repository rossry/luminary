...
from luminary.geometry.primitives import Vector, OrientedSegment
...
class TestBeam:
    ...
    def test_beam_initialization(self):
        ...
        extent_segments = [
            OrientedSegment({"port": Point(0, 10), "starboard": Point(5, 10)})
        ]
        beam = Beam(
    ...
    def test_beam_vertices_calculation(self):
        ...
        forward_starboard = vertices[2]  # from extent_segments[0] starboard
        forward_port = vertices[3]  # from extent_segments[0] port
    ...
    def test_beam_sample_generation(self):
        ...
        extent_segments = [
            OrientedSegment({"port": Point(0, 20), "starboard": Point(5, 20)})
        ]
        beam = Beam(
            extent_segments=extent_segments,
            beam_index=0,
    ...
    def test_beam_dual_extents_supported(self):
        ...
        # Dual extent segments should work
        extent_segments = [
            OrientedSegment({"port": Point(0, 10), "starboard": Point(5, 10)}),
            OrientedSegment({"port": Point(0, 20), "starboard": Point(5, 20)}),
        ]
        ...
        assert len(beam.extent_segments) == 2
    ...
    def test_beam_too_many_extents_not_supported(self):
        ...
        # Three extent segments should raise error
        extent_segments = [
            OrientedSegment({"port": Point(0, 10), "starboard": Point(5, 10)}),
            OrientedSegment({"port": Point(0, 20), "starboard": Point(5, 20)}),
            OrientedSegment({"port": Point(0, 30), "starboard": Point(5, 30)}),
        ]

        with pytest.raises(
            NotImplementedError, match="Currently only 1 or 2 extents supported"
        ):
            Beam(
                extent_segments=extent_segments,
                beam_index=0,
    ...
    def test_beam_point_in_geometry(self):
        ...
        extent_segments = [
            OrientedSegment({"port": Point(0, 10), "starboard": Point(10, 10)})
        ]
        beam = Beam(
            extent_segments=extent_segments,
            beam_index=0,
        ...
        assert beam.is_inside(inside_point)

        # Point outside beam should return False
        outside_point = Point(15, 5)
        assert not beam.is_inside(outside_point)
    ...
    def test_beam_alternation_pattern(self):
        ...
        extent_segments = [
            OrientedSegment({"port": Point(0, 10), "starboard": Point(2, 10)})
        ]
    ...
    def test_beam_color_consistency(self):
        ...
        color1 = beam1._adjust_color_brightness(
            "#FF0000", beam1.get_fill_color_multiplier()
        )
        color2 = beam2._adjust_color_brightness(
            "#FF0000", beam2.get_fill_color_multiplier()
        )

...
class TestBeamIDs(unittest.TestCase):
    ...
    def test_beam_id_creation(self):
        ...
        extent_segment = OrientedSegment(
            {"port": port_point, "starboard": starboard_point}
        )
    ...
    def test_beam_svg_with_pattern_colors(self):
        ...
        extent_segment = OrientedSegment(
            {"port": forward_port, "starboard": forward_starboard}
        )
        ...
        svg_elements_with_pattern = beam.get_svg("#FF0000", beam_colors)
        ...
        self.assertIn(pattern_color.to_svg_str(), svg_content_with_pattern)
    ...
    def test_beam_svg_fallback_to_base_color(self):
        ...
        svg_elements = beam.get_svg("#0000FF", beam_colors)
        svg_content = "".join(svg_elements)

        # Should use adjusted base color, not pattern color
        self.assertIn('id="beam_8:0:0:3"', svg_content)
        self.assertNotIn(Color(0.5, 0.3, 180.0).to_svg_str(), svg_content)
    ...
    def test_beam_id_string_format(self):
        ...
        svg_elements = beam.get_svg("#00FF00")
        svg_content = "".join(svg_elements)
    ...
    def test_beam_basis_point(self):
        ...
        expected_basis = Point(10, 21.0)  # anchor + 0.5 * forward (|forward| = 2)

"""Raindrop ripples appear on the surface, expanding outwards and then vanishing.
...
class RaindropsSpherical(Pattern):
    """Raindrop ripples expand outwards across the spherical dome, then vanish."""

    name = "raindrops_spherical"
    description = (
        "Raindrop ripples appear on the surface, expanding outwards and vanishing."
    )
    ...
    def render(self, lights: np.ndarray, t: float) -> np.ndarray:
        ...
        for drop_idx in range(earliest_drop_idx, latest_drop_idx + 1):
            ...
            # Avoids the very poles; elevation via arccos(uniform(-0.9, 0.9)).
            rnd = seeded_random(f"raindrops_spherical-{drop_idx}", 2)
            ...
            # Skip if ripple has already expanded past this point
            if current_ripple_radius > max_radius:
                continue

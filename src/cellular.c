...
int turing_min_var(turing_vector_t* vv) {
    int arg_min_var = 0;
    double min_var = 1;
    
    for (int ii=0; ii<vv->n_scales; ++ii) {
        if (vv->scale[ii].activ == 0 && vv->scale[ii].inhib == 0) {
            // pass
        } else {
            min_equals1_f(
                &min_var,
                fabs(vv->scale[ii].activ - vv->scale[ii].inhib),
                &arg_min_var,
                ii
            );
        }
